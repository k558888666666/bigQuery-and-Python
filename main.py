# install google-cloud-bigquery and pandas and pandas_gbq
...
def result_to_bigquery():#upload reesult.csv
    ...
    print("uploaded myblood")


names_to_bigquery() #data to bigquery
...
with open('result.csv', 'w+') as result:
    ...
    for i in range(len(years)):
        ...
        for line in results:
            if line.gender == 'F' and len(female_result) < 100 :
                female_result.append(','.join([line.name, str(line.count)]))
            if line.gender == 'M' and len(male_result) < 100 :
                male_result.append(','.join([line.name, str(line.count)]))
            if len(female_result) >= 100 and len(male_result) >= 100:
                break
        for i in range(100):
            result.write(male_result[i])
            result.write(',')
            result.write(female_result[i])
            if i < 99:
                result.write(',')
        result.write('\n')
        print('wait for it!')

#print(len(female_result),len(male_result)) just debug for right values
